...
def main():
    Lines = AoC.Init("data/day24.txt", test=False, nolines=True)
    inputs, gates = parse_input(Lines)
    print(get_output_value(simulate_gates(inputs, gates)))
    swap_outputs("z12", "djg", gates)
    swap_outputs("z19", "sbg", gates)
    swap_outputs("mcq", "hjm", gates)
    swap_outputs("z37", "dsd", gates)
    print_z_exprs(gates)
