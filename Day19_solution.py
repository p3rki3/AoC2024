...
def match_pattern(cloth, pattern):
    if pattern:
        ret_val = 0
        if pattern in SubPattern:
            return SubPattern[pattern]
        for cloth in Cloth:
            ret_val += match_pattern(Cloth, pattern[len(cloth):]) if pattern.startswith(cloth) else 0
...
def solve() -> tuple:
    ...
    for pattern in Patterns:
        pattern_match_count = match_pattern(Cloth, pattern)
