...
def solve():
    part1, part2 = [], []
    for start_p in [ Point(x, y) for y, line in enumerate(Grid) for x, c in enumerate(line) if c == 0 ]:
        end_of_trail = []
        walk_path(start_p.x, start_p.y, -1, end_of_trail)
        part1.append(len(set(end_of_trail)))
        part2.append(len(end_of_trail))
    return sum(part1), sum(part2)

Lines, numlines, linelen = AoC.Init("data/day10.txt")
